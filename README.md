# 👋 Jeffrey Samuel Schulman Jr.

### 🛡️ Enterprise IT & Cybersecurity Stragegy, Engineering, and Operations
### 🎓 Ph.D. Student & Adjunct Instructor
### 🔬 Information Security, Explainable AI, and Active Inference Researcher
### 💼 Board Member

Welcome! I'm **Jeff Schulman**, a cybersecurity professional at **Netskope**, Ph.D. student and adjunct instructor at **Penn State**, and founder of **Manteio Company**, specializing in security strategy and fractional vCISO services. My work spans **human-computer interaction (HCI), information security, explainable AI, deception**, and **active inference**.

---

## 🔬 Research & Interests  

- **Dark Patterns & Deception** – Investigating manipulative UI/UX tactics and their impact, particularly on vulnerable populations.  
- **Explainable AI & Human-Centered Security** – Exploring how AI models can be made transparent and trustworthy.  
- **Active Inference & Cognitive Security** – Applying Bayesian principles to cybersecurity decision-making and adversarial modeling.  
- **Open-Source & Digital Rights** – Advocating for ethical design and security in open-source ecosystems.  
- **Cybersecurity** – Researching entropy, identity management, and cyber resilience.  

📄 **Recent Publications**:
- **Kumar, P. C., Schulman, J. S., Albargi, F., Bhattacharyya, S., Dong, H., & Liu, Z. (2025).**  
  *Ungrading as a Pedagogy for Teaching Qualitative Research Methods in Computing.*  
  In *Proceedings of the 56th ACM Technical Symposium on Computer Science Education V. 1*, 631–637.  
  [🔗 DOI: 10.1145/3641554.3701814](https://doi.org/10.1145/3641554.3701814)  
  Published by **Association for Computing Machinery (ACM)**, New York, NY, USA. 
- *"Focusing on Goals and Objectives: Updating a Principle of Marine Corps Leadership"* – Marine Corps Gazette (December 2024)  
- *"Entropy: An Essential Component of Cryptographic Security"* – Journal of Cybersecurity Awareness and Education  

➡️ **Full list:** [Curriculum Vitae](https://www.jeffschulman.com/cv)  

---

## 🏛️ Professional Background  

- **Director of Government Services, Netskope** – Leading secure cloud operations for FedRAMP High & PBMM environments, ensuring compliance and mission-critical support for government clients.  
- **Founder, Manteio Company** – Security strategy, AI-driven risk management, and compliance solutions.  
- **Adjunct Faculty, Penn State University** – Teaching cybersecurity, networking, and risk management.  
- **Former CISO (Acting) & Cybersecurity Architect, Penn State Applied Research Lab** – Led security operations and compliance for a $400M+ research enterprise.  
- **USMC Veteran** –  Cybersecurity and IT operations supporting military and defense missions.  

👨‍💻 **Past experience** includes work with DoD/IC (AFRICOM, NSA, NGA, USFK) and enterprise IT security teams.  

---

## 📚 Teaching & Mentorship  

- **Courses Taught:** Networking & Telecommunications, Defense Message Dissemination System
- **Guest Lectures:** Influence operations, AI safety, and security misconfigurations  
- **Mentor:** STEM & cybersecurity students, OSPO (Open-Source Program Office) co-founder at Penn State  

---

## 🔗 Connect & Collaborate  

- 🔍 [Website](https://www.jeffschulman.com/)  
- 📝 [Google Scholar](https://scholar.google.com/citations?user=XXXXXXX)  
- 🏛️ [Penn State Profile](https://ist.psu.edu/directory/jss6099)  
- 🆔 [ORCID](https://orcid.org/my-orcid?orcid=0009-0004-9806-4113)  
- 🐦 [Twitter](https://twitter.com/jeffschulman)  
- 💼 [LinkedIn](https://linkedin.com/in/jeffschulman)
- 📊 [RateMyProfessors](https://www.ratemyprofessors.com/professor/2780667)   

🚀 **Open to collaborations in information security, HCI, CSCW,  and active inference research**  

---
